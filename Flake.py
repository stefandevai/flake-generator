...
class Flake:
    ...
    def draw(self, st, win):
        self.steps = st
        self.screen_ref = win
        self.scale_factor = self.__get_scale_factor()
    ...
    def __draw_hexaflake(self, steps, rd, ct):
        if steps <= 0:
            ...
            for i in range(0, self.num_vertices):
                cx = ct[0] + rd*math.cos(math.radians((self.angle_base)*i + ROTATION)) - rd*self.scale_factor*math.cos(math.radians((self.angle_base)*i + ROTATION))
                cy = ct[1] + rd*math.sin(math.radians((self.angle_base)*i - ROTATION)) + rd*self.scale_factor*math.sin(math.radians((self.angle_base)*i + ROTATION))
    ...
    def __draw_pentaflake(self, steps, rd, ct, inv):
        if steps <= 0:
            ...
            for i in range(0, self.num_vertices):
                cx = ct[0] + rd*math.cos(math.radians((self.angle_base)*i + ROTATION)) - rd*self.scale_factor*math.cos(math.radians((self.angle_base)*i + ROTATION))
                cy = ct[1] + rd*math.sin(math.radians((self.angle_base)*i - inv*ROTATION)) + inv*rd*self.scale_factor*math.sin(math.radians((self.angle_base)*i + ROTATION))
